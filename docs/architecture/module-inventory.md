...
- 2026-04-20 — Task #35 (post-#6 audit punch-list, item 1): Categorized the three `account-request-*` components in §2 row notes and §3 layout one-liners — `account-request-kms` (Cloudposse-wrapper, `cloudposse/kms-key/aws` v0.12.2), `account-request-notifications` (Cloudposse-wrapper, `cloudposse/sns-topic/aws` v1.2.0), `account-request-queue` (Vanilla — `aws_sqs_queue` main + DLQ, no backing module). No source change; readers can now go from row → component dir → category at a glance.
